...
class UGATIT(object) :
    def __init__(self, args):
        ...
        self.logger = TdLogger(
            ...
            (args.logger_username, args.logger_password), 
            group_prefix=f'UGAT-IT-{args.dataset}-{args.batch_size}',
            disabled=args.logger_disabled)
    ...
    def train(self):
        ...
        for step in range(start_iter, self.iteration + 1):
            ...
            if step % self.print_freq == 0:
                ...
                a2b_path = os.path.join(self.result_dir, self.dataset, 'img', 'A2B_%07d.png' % step)
                b2a_path = os.path.join(self.result_dir, self.dataset, 'img', 'B2A_%07d.png' % step)
                cv2.imwrite(a2b_path, A2B * 255.0)
                cv2.imwrite(b2a_path, B2A * 255.0)
                self.logger.sendBlobFile(a2b_path, os.path.basename(a2b_path), f"/validation/{self.logger.group_prefix}/{os.path.basename(a2b_path)}", "validationImage")
                self.logger.sendBlobFile(b2a_path, os.path.basename(b2a_path), f"/validation/{self.logger.group_prefix}/{os.path.basename(b2a_path)}", "validationImage")
                self.genA2B.train(), self.genB2A.train(), self.disGA.train(), self.disGB.train(), self.disLA.train(), self.disLB.train()
